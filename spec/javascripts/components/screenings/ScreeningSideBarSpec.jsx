...
describe('ScreeningSideBar', () => {
  ...
  it('renders side-bar-error class when there is an error', () => {
    const navbar = shallow(<ScreeningSideBar participants={participants} error={true}/>, {disableLifecycleMethods: true})
    expect(navbar.find('.side-bar-error').exists()).toEqual(true)
  })

  it('renders side-bar-no-error class when there is not an error', () => {
    const navbar = shallow(<ScreeningSideBar participants={participants} error={false}/>, {disableLifecycleMethods: true})
